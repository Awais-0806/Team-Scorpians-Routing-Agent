...
AMD ROCm Python FastAPI Docker Hackathon
...
🏗️ Architecture
User Query │ ▼ Heuristic Pre-Filter ──(obviously hard?)──► Escalate to Cloud │ ▼ Local Gemma 4 E4B (vLLM on ROCm) ── asked TWICE at temperature 0.7 │ ▼ Self-Consistency Check (Compare both answers) │ ├─── Answers AGREE ───► Return Local Answer ── (COST: 0 TOKENS) ✅ │ └─── Answers DISAGREE ───► Escalate to Fireworks AI │ ▼ Gemma 4 31B IT (Cloud) │ ▼ Return Cloud Answer (COST: REAL TOKENS)
